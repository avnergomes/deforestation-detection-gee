"""Quick test script with forced module reload."""

import sys
import importlib

# Force reload of the module to get latest changes
if 'deforestation_detector' in sys.modules:
    importlib.reload(sys.modules['deforestation_detector'])
...
def test_detector():
    ...
    print("=" * 60)
    print("🧪 TESTING DEFORESTATION DETECTOR (FORCED RELOAD)")
    print("=" * 60)
    ...
    try:
        ...
        if len(scenes) > 0:
            print(f"  ✅ SUCCESS! Found {len(scenes)} scenes")
    ...
    if scenes:
        print("✅ Multi-satellite support IS WORKING!")
        print("\n💡 Next steps:")
        print("   1. Stop Streamlit (Ctrl+C)")
        print("   2. Restart: streamlit run streamlit_app.py")
        print("   3. Upload locations CSV")
        print("   4. Click 'Run analysis'")
    else:
        print("⚠️  No scenes prepared - check console output above")
